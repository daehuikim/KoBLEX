#!/usr/bin/env python3
"""
FLARE Legal Reasoning (Qwen)

This script implements FLARE (Forward-Looking Active REtrieval) 
reasoning for legal questions using Qwen model with vLLM.
"""
...
class LlmGenerator:
    """vLLM generator for FLARE reasoning with Qwen."""
...
def decide_retrieval(llm_gen: LlmGenerator, token_lps: List[float], token_threshold: float = -0.60) -> Tuple[str, bool]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="FLARE legal reasoning using Qwen model"
    )
    ...
    parser.add_argument(
        "--output_file", 
        type=str, 
        default="flare_qwen_results.jsonl",
