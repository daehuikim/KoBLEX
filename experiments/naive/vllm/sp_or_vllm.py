#!/usr/bin/env python3
"""
Simple Prompt with One-time Retrieval Question Answering Generator (VLLM)

This script generates legal answers using simple prompts with one-time retrieval using VLLM.
...
class LlmGenerator:
    ...
    def _generate(self, prompts):
        ...
        for idx, prompt in enumerate(prompts):
            messages = [
                {"role": "system", "content": SP_OR_SYSTEM_PROMPT},
...
def make_qa_prompts(data_list: List[Dict], retriever, context_list: List[str], n_hops: int = None):
    ...
    for entry in data_list:
        ...
        context_str = "\n".join(context_strs)
        prompt = SP_OR_QA_PROMPT.substitute(question=temp_question, context_str=context_str)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Legal QA using simple prompts with one-time retrieval using VLLM"
    )
    ...
    with open(args.output, 'w', encoding='utf-8') as fw:
        for entry, prediction, combi in zip(data_list, results, combinations):
            record = {
                ...
                "answers": [{"answer": prediction[0].strip(), "logp": prediction[1], "comb": combi}]
            }
