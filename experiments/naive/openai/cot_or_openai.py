#!/usr/bin/env python3
"""
Chain-of-Thought with One-time Retrieval Question Answering Generator (OpenAI)

This script generates legal answers using chain-of-thought reasoning with one-time retrieval using OpenAI API.
...
async def create_prompts(data_list: List[Dict[str, Any]], retriever, context_list: List[str], n_hops: int = None) -> List[str]:
    ...
    for entry in data_list:
        ...
        full_prompt = f"{COT_OR_SYSTEM_PROMPT}\n\n{COT_OR_QA_PROMPT.substitute(question=temp_question, context_str=context_str)}"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Legal QA using chain-of-thought reasoning with one-time retrieval using OpenAI API"
    )
