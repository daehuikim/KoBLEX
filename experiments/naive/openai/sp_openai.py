#!/usr/bin/env python3
"""
Simple Prompt Question Answering Generator (OpenAI)

This script generates legal answers using simple prompts with OpenAI API.
...
from ..prompts.naive_prompt import SYSTEM_PROMPT, SP_PROMPT
...
async def create_prompts(data_list: List[Dict[str, Any]]) -> List[str]:
    ...
    for item in data_list:
        ...
        full_prompt = f"{SYSTEM_PROMPT}\n\n{SP_PROMPT.substitute(question=full_question)}"
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Legal QA using simple prompts with OpenAI API"
    )
