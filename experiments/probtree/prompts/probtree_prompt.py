...
# Probtree Question Answering Prompts
...
CA_SYSTEM_PROMPT = """You are given a context and a legal question. Use only the information from the provided context to write a concise and accurate legal answer to the question. Do not include any markdown formatting. Your response must end with a single sentence beginning with:
"So the answer is:"
Place your final conclusion only after that sentence. Do not repeat or summarize the context or question. Stay strictly within the information given in the context."""
...
CA_TEMPLATE = Template("""<EXAMPLE>
Question: 갑이 군 복무 중 납부한 기여금은 어떻게 처리되는가?
Context: 군인연금법 4조 기여금의 반환 제4조(기여금의 반환) 군인이었던 사람으로서 이 법에 따른 급여를 받을 권리가 없는 사람 또는 그 유족에 대해서는 그 군인이 복무할 때 낸 기여금에 대통령령으로 정하는 이자를 합한 금액을 반환한다.
Answer: 군인연금법에 따르면, 군인이었던 사람으로서 급여를 받을 권리가 없는 사람 또는 그 유족에 대해서는 그 군인이 복무할 때 낸 기여금에 대통령령으로 정하는 이자를 합한 금액을 반환한다. So the answer is: 갑이 군 복무 중 납부한 기여금은 그 기여금에 대통령령으로 정하는 이자를 합한 금액으로 반환된다.
