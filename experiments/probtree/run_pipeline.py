#!/usr/bin/env python3
"""
Probtree Pipeline Runner

This script orchestrates the complete probtree pipeline:
1. Generate hierarchical question decomposition trees
2. Use trees to perform probtree question answering
"""
...
def main():
    """Main function to run the complete probtree pipeline."""
    parser = argparse.ArgumentParser(
        description="Run the complete probtree pipeline"
    )
    ...
    print("🚀 Starting Probtree Pipeline")
    ...
    # Step 2: Run probtree question answering
    if args.use_openai:
        probtree_cmd = [
            sys.executable, "probtree_openai.py",
        ...
        probtree_cmd = [
            sys.executable, "probtree_vllm.py",
            "--input", str(tree_output),
            "--context", args.context,
            "--output", args.output,
            "--model_name", args.model_name,
    ...
    if not run_command(probtree_cmd, "Step 2: Run probtree question answering"):
        print("❌ Pipeline failed at probtree step")
        return 1
