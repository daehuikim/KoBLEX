#!/usr/bin/env python3
"""
Probtree Question Answering Generator (VLLM)

This script performs hierarchical question decomposition tree-based question answering using VLLM.
...
from utils import load_jsonl, save_jsonl, setup_retriever, avg_logprob
...
def probtree(llm, retriever, entry, context_list):
    ...
    def post_order(idx):
        ...
        if not is_leaf:
            ca_out, ca_lps = llm._generate(ca_prompt, CA_SYSTEM_PROMPT)
        ...
        if not is_leaf:
            ...
            if prefix in ca_out:
                full_ids = llm.tokenizer.encode(ca_out, add_special_tokens=False)
                pref_ids = llm.tokenizer.encode(ca_out.split(prefix, 1)[0] + prefix, add_special_tokens=False)
                start_idx = len(pref_ids)
                post_lps = ca_lps[start_idx:]
            else:
                post_lps = ca_lps
            scores["ca"] = avg_logprob(post_lps)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Probtree question answering using hierarchical question decomposition trees with VLLM"
    )
    ...
    for entry in tqdm(data_list, desc="Processing entries"):
        trace, final_answer, comb = probtree(llm, retriever, entry, context_list)
